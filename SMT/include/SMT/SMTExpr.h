...
#define DECLARE_BINARY_OPERATION_T_EXPR(X, T) \
    ...
    friend std::ostream & operator<<(std::ostream &, SMTExpr const &);
       ...
       bool SMTExprToStream(llvm::raw_fd_ostream& Out);
...
#define DECLARE_BASIC_OPERATION(X) \
    ...
    SMTExpr basic_zext(unsigned Sz) const;

    SMTExpr basic_sext(unsigned Sz) const;
...
#define DECLARE_ARRAY_OPERATION(X) \
    ...
    // "qe2" is based on "Playing with quantified satisfaction, LPAR 15"
    //     that uses "model based projection" proposed in CAV 14.

    // We use the name "forget" because this procedure(or its variant)
    // is often called "forget operator" both by
    // abstract interpretation community and artificial intelligence community.
    SMTExpr forgetVars();

    SMTExpr forgetVar(SMTExpr& Var);

    SMTExpr forgetVars(SMTExprVec& Vars);
...
private:
    /********************
      *  Generalized consequence finding or symbolic abstraction. Given P(x,y,z), find its
      *  strongest consequence/best abstraction/strongest necessary condition in an abstract domain.
      *
      *
      *  Usage: create an Empty SMTExprVec Res and pass to the function
      *         currently, only support single variable query, and single objective
      *
     **********************/

    // get the variables of current expr
    bool getExprVars(SMTExprVec& Vars);

    // light weight, sound bound
    bool soundAbstractInterval(SMTExpr& Query, SMTExprVec& Res);

    void bestAbstractInterval(SMTExpr& Query, SMTExprVec& Res, std::string Method = "opt", int Timeout = 5000);

    // compute with linear search
    void bestAbstractIntervalWithLinearSearch(SMTExpr& Query, SMTExprVec& Res, int Timeout = 5000);

    // compute with SMT optimization
    void bestAbstractIntervalWithOPT(SMTExpr& Query, SMTExprVec& Res, int Timeout = 5000);

    // compute quantified SMT solving
    void bestAbstractIntervalWithQSMT(SMTExpr& Query, SMTExprVec& Res, int Timeout = 5000);

    // dump the symbolic optimization constraints
    void dumpOptimizeObjectives(SMTExprVec& Queries);
...
public:
    ...
    SMTExprVec diff(const SMTExprVec& Vars);
    ...
    bool SMTExprVecToStream(llvm::raw_ostream& Out);
};
