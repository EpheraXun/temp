...
protected:
    ...
    bool is_builtin_option(symbol const & s) const {
        return 
            ...
            s == m_random_seed || s == m_verbosity || s == m_inc_qfbv || s == m_global_decls || s == m_global_declarations ||
...
public:
    ...
    virtual void set_next_arg(cmd_context & ctx, rational const & val) {
        // Pinpoint {
        if (m_option == m_inc_qfbv) {
            set_inc_qfbv(to_unsigned(val));
        }
        // }
...
public:
    ...
    virtual void set_next_arg(cmd_context & ctx, symbol const & opt) { 
        ...
        // Pinpoint {
        else if (opt == m_inc_qfbv) {
            print_unsigned(ctx, get_inc_qfbv());
        }
        // }
