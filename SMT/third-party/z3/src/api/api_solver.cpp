...
extern "C" {
    ...
    // Pinpoint: add get_bv_features API for z3 sovler object
    // TODO: use tactic instead of a API for solver
    void Z3_API Z3_solver_get_bv_features(Z3_context c, Z3_solver s, std::vector<double>& features) {
            Z3_TRY;
            LOG_Z3_solver_get_assertions(c, s);
            RESET_ERROR_CODE();
            init_solver(c, s);
            to_solver_ref(s)->collect_bv_info(features);
            Z3_CATCH;
    }
