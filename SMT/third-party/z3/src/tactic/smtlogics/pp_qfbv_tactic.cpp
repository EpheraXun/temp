...
static tactic * mk_pp_qfbv_preamble(ast_manager& m, params_ref const& p) {
    ...
    return
            and_then(
                ...
                // pinpoint: disable ackermannization(it can eliminate ininterpreted functions, which we do not use.) 
    ...
    tactic * st = main_p(and_then(preamble_st,
				      ...
				      the UFs can be eliminated by eager ackermannization in the preamble.
                                  */
                                  ...
                                  /*
                                  pinpoint: disable `qfbv_eq_probe` and use a uniform stragety for solving QF_BV formulas
