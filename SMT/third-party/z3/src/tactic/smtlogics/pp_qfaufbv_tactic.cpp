...
tactic * mk_pp_qfaufbv_light_tactic(ast_manager& m, params_ref const & p, tactic* sat, tactic* smt) {
    ...
    // use smt_tactic in case the preamble_st fail to convert the problem to a purely QF_BV problem
    // it seems many cases from KLEE will fall into this catetory...
    tactic * st = main_pp(and_then(preamble_st,
    		                cond(mk_is_qfbv_probe(),
                                    and_then(mk_bit_blaster_tactic(m),
                                            when(mk_lt(mk_memory_probe(), mk_const_probe(MEMLIMIT)),
...
tactic * mk_pp_qfaufbv_light_tactic(ast_manager & m, params_ref const & p) {
    ...
    return mk_pp_qfaufbv_light_tactic(m, p, new_sat, mk_smt_tactic());
}
...
tactic * mk_pp_qfaufbv_tactic(ast_manager & m, params_ref const & p) {
    ...
    return mk_pp_qfaufbv_tactic(m, p, new_sat, mk_smt_tactic());
}
