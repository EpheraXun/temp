...
#include"pp_inc_bv_solver.h"
#include"pp_qfbv_light_tactic.h"
#include"pp_qfbv_approximation_tactic.h"
//}
...
public:
    ...
    virtual solver * operator()(ast_manager & m, params_ref const & p, bool proofs_enabled, bool models_enabled, bool unsat_core_enabled, symbol const & logic) {
        // By default, get_inc_qfbv() == 0. And the default tactic will be taken.
        unsigned inc_tactic = get_inc_qfbv();
        if (inc_tactic == 0) {
            symbol l;
        ...
        // Use qfbv_tactic to create the incremental solver, i.e., the second parameter of mk_combined_solver(p1, p2)
        } else if (inc_tactic == 1) {
            symbol ll("QF_BV");
            return mk_combined_solver(mk_tactic2solver(m, mk_qfbv_tactic(m, p), p, proofs_enabled, models_enabled, unsat_core_enabled, ll),
                                      mk_tactic2solver(m, mk_qfbv_tactic(m, p), p, proofs_enabled, models_enabled, unsat_core_enabled, ll),
                                      p);

        // Use pp_qfbv_tactic to create the incremental solver
        } else if (inc_tactic == 2) {
            symbol ll("QF_BV");
            return mk_combined_solver(mk_tactic2solver(m, mk_pp_qfbv_tactic(m, p), p, proofs_enabled, models_enabled, unsat_core_enabled, ll),
                                      mk_tactic2solver(m, mk_pp_qfbv_tactic(m, p), p, proofs_enabled, models_enabled, unsat_core_enabled, ll),
                                      p);
        } else if (inc_tactic == 3) {
            symbol ll("QF_BV");
            return mk_combined_solver(mk_tactic2solver(m, mk_pp_qfbv_tactic(m, p), p, proofs_enabled, models_enabled, unsat_core_enabled, ll),
                                      mk_pp_inc_bv_solver(m, p),
                                      p);

        } else if (inc_tactic == 4) {
            symbol ll("QF_BV");
            return mk_combined_solver(mk_tactic2solver(m, mk_pp_qfbv_light_tactic(m, p), p, proofs_enabled, models_enabled, unsat_core_enabled, ll),
                                      mk_tactic2solver(m, mk_pp_qfbv_light_tactic(m, p), p, proofs_enabled, models_enabled, unsat_core_enabled, ll),
                                      p);
        } else if (inc_tactic == 5) {
            symbol ll("QF_BV");
            return mk_combined_solver(mk_tactic2solver(m, mk_pp_qfbv_light_tactic(m, p), p, proofs_enabled, models_enabled, unsat_core_enabled, ll),
                                      mk_tactic2solver(m, mk_pp_qfbv_approximation_tactic(m, p), p, proofs_enabled, models_enabled, unsat_core_enabled, ll),
                                      p);
        }
