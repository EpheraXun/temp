...
#define CONFLICT_CLEANUP() {                    \
    ...
    */
    void solver::simplify_problem() {
        ...
        if (m_config.m_enable_scc) {
            m_scc();
            CASSERT("sat_simplify_bug", check_invariant());
        }
        
        // Pinpoint crash fix. z3 iss939(double free).
        // Farewell m_simplifier..
        /*
        m_simplifier(false);
        CASSERT("sat_simplify_bug", check_invariant());
        CASSERT("sat_missed_prop", check_missed_propagation());
        ...
        // NOTE: enable_simplifier is false by default.
                It may cause crash.
        */
        if (m_config.m_enable_simplifier) {
            m_simplifier(false);
            CASSERT("sat_simplify_bug", check_invariant());
            CASSERT("sat_missed_prop", check_missed_propagation());
        ...
        // m_enable_probing: apply failed literal probing(default true)
        if (m_config.m_enable_probing) {
            // If we are using pp_qfbv_light_tactic, this option will be false.
            m_probing();
            CASSERT("sat_missed_prop", check_missed_propagation());
            CASSERT("sat_simplify_bug", check_invariant());
        }
