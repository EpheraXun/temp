...
// pinpoint {
// Collect info of QF_BV constraints
void solver::collect_bv_info(std::vector<double>& features) {
    expr_ref_vector fmls(get_manager());
    get_assertions(fmls);
    ast_pp_util visitor(get_manager());

    unsigned n = fmls.size();
    expr* const* es = fmls.c_ptr();

    // use the hashcode of expr as key.
    // expr_info_memo
    //
    std::vector<unsigned> toadd;
    for (unsigned i = 0; i < n; ++i) {
        unsigned hash_code = es[i]->hash();
        if (expr_info_memo[hash_code].collected == true) {
            //std::cout << "cache hit \n";
            toadd.push_back(hash_code);
            /*
            for (unsigned j = 0; j < 32; j++) {
                features[j] += expr_info_memo[es[i]->hash()].vec[j];
            }*/
        } else {
            // TODO: pass reference of features, expr_info_memo to visit_bv, instead of creating tmp vector.
            std::vector<double> tmp(32, 0);
            visitor.coll.visit_bv(es[i], tmp);
            for (unsigned j = 0; j < 32; j++) {
                features[j] += tmp[j];
                expr_info_memo[hash_code].vec[j] = tmp[j];
            }
            expr_info_memo[hash_code].collected = true;
        }
    }
    for (unsigned i = 0; i < toadd.size(); i++) {
        scoped_vector& memo = expr_info_memo[toadd[i]];
        for (unsigned j = 0; j < 32; j++) {
        	features[j] += memo.vec[j];
        }
    }
    features[32] += visitor.coll.get_num_decls();
}
// }
